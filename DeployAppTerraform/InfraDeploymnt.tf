...
resource "azurerm_public_ip" "app-g1-publicip" {
  count               = 3
  name                = "app-g1-publicip-${count.index + 1}"
  location            = "West Europe"
  resource_group_name = "RG-LABS-01"
...
resource "azurerm_network_interface" "app-g1-nic" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.app-g1-publicip[count.index].id
  }
...
resource "azurerm_network_interface_security_group_association" "NIC-NSG" {
  count                     = 3
  network_interface_id      = azurerm_network_interface.app-g1-nic[count.index].id
...
resource "azurerm_linux_virtual_machine" "app-deploy-vms" {
  ...
  location              = "West Europe"
  resource_group_name   = "RG-LABS-01"
  network_interface_ids = [azurerm_network_interface.app-g1-nic[count.index].id]
